...
const SpeechToText = ({ onTranscriptChange, onStopRecording }) => {
  ...
  useEffect(() => {
    if ("webkitSpeechRecognition" in window) {
      ...
      speechRecognition.onresult = (event) => {
        ...
        onTranscriptChange(finalTranscript); // Passing the transcript to the parent
      };
  ...
  const stopRecording = () => {
    if (recognition) {
      ...
      recognition.stop();
      onStopRecording(); // Notify the parent that recording has stopped
    }
  };
...
const SpeechToTextAndCloudFlare = () => {
  ...
  const handleTranscriptChange = (newTranscript) => {
    setTranscript(newTranscript); // Updating the parent state when transcript changes
  };

  const handleStopRecording = () => {
    // Trigger CloudFlare API when recording stops
